  ...
  import { DEFAULT_TRACK, activeTrack, resetTrack, setTrack, trackFile, trackMeta } from "../engine/track.js";

  // Every data file is per-track and resolves through trackFile(). Tracks share
  // nothing, so there is no repo-global data path left in this module.
  ...
  const state = {
    ...
    feed: null,
    feedStatus: null,
    // Set when the stored track could not be loaded and the default was used
    // instead; surfaced by renderDataWarning.
    trackFallback: null,
    frequency: null,
  ...
  function switchWorkspace(workspace, targetHash = "") {
    ...
    // On a non-default track the eyebrow carries the track, so no screen can be
    // mistaken for the main curriculum's.
    const track = activeTrack();
    if (eyebrow) {
      eyebrow.textContent =
        track === DEFAULT_TRACK ? meta.eyebrow : `${trackMeta(track).short} · ${meta.eyebrow}`;
    }
  ...
  function loadTrackFiles(track) {
    return Promise.all([
      fetchJson(trackFile("progress", track)),
      fetchJson(trackFile("scoring", track)),
      fetchJson(trackFile("curriculum", track)),
      fetchJson(trackFile("stages", track)),
      fetchJson(trackFile("skills", track)),
      fetchJson(trackFile("patterns", track)),
      fetchJson(trackFile("mistakes", track)),
    ]);
  }
  ...
  async function loadData() {
    ...
    let files;
    try {
      files = await loadTrackFiles(track);
    } catch (error) {
      if (track === DEFAULT_TRACK) throw error;
      // The stored track's files are gone (never generated, or deleted). Falling
      // back beats failing: the load-failure page has no track switch on it, so
      // a bad stored track would otherwise trap the dashboard permanently.
      state.trackFallback =
        `The ${trackMeta(track).label} track could not be loaded (${error.message}). ` +
        `Showing ${trackMeta(DEFAULT_TRACK).label} instead — run \`python3 scripts/generate_blind75_track.py\` to rebuild it.`;
      resetTrack();
      track = DEFAULT_TRACK;
      files = await loadTrackFiles(track);
    }
    const [progress, scoring, curriculum, stages, skills, patternIndex, mistakes] = files;
    ...
    state.frequency = await fetchJson(trackFile("frequency", track)).catch(() => null);
  }
  ...
  function ensureDependencyGraph() {
    if (!state.graphPromise) {
      state.graphPromise = fetchJson(trackFile("dependencyGraph"))
        .then((graph) => {
  ...
  function renderDataWarning() {
    ...
    // A failed track load outranks every other warning: everything on screen is
    // from a different curriculum than the one that was asked for.
    if (state.trackFallback) {
      $("#data-warning-text").textContent = state.trackFallback;
      banner.hidden = false;
      return;
    }
  ...
  // The rail's track segments plus the source card beneath them: together they
  // answer "which curriculum am I looking at, and which file is that?".
  function renderTrackSwitch() {
    ...
    document.querySelectorAll(".track-option").forEach((button) => {
      ...
      button.title = isActive
        ? `${trackMeta(button.dataset.track).label} (active)`
        : `Switch to ${trackMeta(button.dataset.track).label}`;
    });
    const sourceFile = $("#source-file");
    if (sourceFile) {
      sourceFile.textContent = trackFile("progress").replace(/^\.\.\//, "");
    }
  }

  function renderAll() {
    renderTrackSwitch();
    $("#last-updated").textContent = `Updated ${state.datasets.progress.last_updated ?? "never"}`;
