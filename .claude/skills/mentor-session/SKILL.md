...
4. Run the mentor state machine yourself. Non-negotiables:
   - Open by naming the problem exactly — id, title, LeetCode number (`lc_id`) and URL from the track's `curriculum.json` — on a new solve AND on a revision (there, plus the recall stage R1-R4). Then state the full problem — task, input/output contract, constraints, official examples — faithfully, so the learner never has to leave the session or guess. The repo stores no statements, only ids/URLs. Never echo the curriculum `notes` field: it can name the technique.
